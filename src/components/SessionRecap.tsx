    ...
    <div className="glass-card rounded-[20px] p-6 md:p-8 h-full flex flex-col overflow-y-auto">
      ...
      <div className="space-y-4 flex-1">
        {session.blocks.map((block, i) => (
          <BlockDetail key={i} block={block} segment={timeline[i]} index={i} total={session.blocks.length} />
        ))}
      </div>
...
function ExerciseName({ name }: { name: string }) {
  ...
  return (
    <Link
      to={to}
      className="text-[13px] font-medium text-link hover:text-link-hover transition-colors underline underline-offset-2 decoration-link/30"
    >
      {name}
    </Link>
  );
