import { ArrowRight, Crown, ListChecks, RotateCcw, Sparkles, Target, TrendingUp, Wand2 } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { Link } from 'react-router';
import { useAuth } from '../contexts/AuthContext.tsx';
import { useDocumentHead } from '../hooks/useDocumentHead.ts';
import { useSubscription } from '../hooks/useSubscription.ts';
import { PricingCards } from './PricingCards.tsx';
import { ScreenshotCarousel } from './ScreenshotCarousel.tsx';

export function PremiumPromoPage() {
  const { t } = useTranslation('marketing');
  const { user } = useAuth();
  const { isPremium } = useSubscription();

  useDocumentHead({
    title: t('premium.page_title'),
    description: t('premium.page_description'),
  });

  return (
    <div className="space-y-0">
      {/* ── 1. Hero ── */}
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-accent/10 via-transparent to-brand/5" />
        <div className="relative z-10 px-6 md:px-10 lg:px-14 py-16 md:py-24">
          <div className="max-w-5xl mx-auto text-center space-y-6">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent/10 border border-accent/20">
              <Crown className="w-4 h-4 text-accent" aria-hidden="true" />
              <span className="text-sm font-semibold text-accent">{t('premium.badge')}</span>
            </div>

            <h1 className="font-display text-4xl sm:text-5xl md:text-6xl font-black text-heading leading-[1.1]">
              {t('premium.hero_heading_1')}
              <br />
              <span className="bg-gradient-to-r from-accent to-brand bg-clip-text text-transparent">
                {t('premium.hero_heading_2')}
              </span>
            </h1>

            <p className="text-lg md:text-xl text-muted max-w-2xl mx-auto leading-relaxed">{t('premium.hero_body')}</p>

            {!user && (
              <div className="pt-2">
                <Link
                  to="/signup"
                  className="inline-flex items-center gap-2 px-8 py-4 rounded-full text-base font-bold text-white bg-accent hover:bg-accent/90 transition-colors"
                >
                  <Sparkles className="w-5 h-5" aria-hidden="true" />
                  {t('premium.hero_cta_signup')}
                </Link>
              </div>
            )}

            {!isPremium && user && (
              <p className="text-sm text-muted">
                <a href="#tarifs" className="text-accent font-semibold hover:text-accent/80 transition-colors">
                  {t('premium.hero_cta_pricing')} <ArrowRight className="w-3.5 h-3.5 inline" aria-hidden="true" />
                </a>
              </p>
            )}
          </div>
        </div>
      </section>

      {/* ── 2. Séances IA sur-mesure ── */}
      <section className="px-6 md:px-10 lg:px-14 py-14 md:py-20 bg-surface-2/50">
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
            <div className="order-2 md:order-1">
              <ScreenshotCarousel
                maxHeight="max-h-[420px]"
                fit="contain"
                dotColor="accent"
                images={[
                  {
                    src: '/images/screenshot-custom-session.webp',
                    alt: t('premium.ai_sessions.carousel_quick_alt'),
                  },
                  {
                    src: '/images/screenshot-custom-detailed.webp',
                    alt: t('premium.ai_sessions.carousel_detailed_alt'),
                  },
                  {
                    src: '/images/screenshot-custom-expert.webp',
                    alt: t('premium.ai_sessions.carousel_expert_alt'),
                  },
                ]}
              />
            </div>

            <div className="space-y-5 order-1 md:order-2">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold tracking-widest uppercase text-accent">
                  {t('premium.ai_sessions.label')}
                </span>
              </div>
              <h2 className="font-display text-3xl md:text-4xl font-black text-heading leading-tight">
                {t('premium.ai_sessions.heading_1')}
                <br />
                {t('premium.ai_sessions.heading_2')}
              </h2>
              <p className="text-muted leading-relaxed">{t('premium.ai_sessions.body')}</p>
              <ul className="space-y-3">
                {[
                  { icon: Wand2, text: t('premium.ai_sessions.bullet_1') },
                  { icon: RotateCcw, text: t('premium.ai_sessions.bullet_2') },
                  { icon: Target, text: t('premium.ai_sessions.bullet_3') },
                ].map((item) => (
                  <li key={item.text} className="flex items-start gap-3">
                    <item.icon className="w-5 h-5 text-accent shrink-0 mt-0.5" aria-hidden="true" />
                    <span className="text-sm text-body">{item.text}</span>
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </div>
      </section>

      {/* ── 3. Programmes IA personnalisés ── */}
      <section className="px-6 md:px-10 lg:px-14 py-14 md:py-20">
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
            <div className="space-y-5">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold tracking-widest uppercase text-accent">
                  {t('premium.ai_programs.label')}
                </span>
              </div>
              <h2 className="font-display text-3xl md:text-4xl font-black text-heading leading-tight">
                {t('premium.ai_programs.heading_1')}
                <br />
                {t('premium.ai_programs.heading_2')}
              </h2>
              <p className="text-muted leading-relaxed">{t('premium.ai_programs.body')}</p>
              <ul className="space-y-3">
                {[
                  { icon: ListChecks, text: t('premium.ai_programs.bullet_1') },
                  { icon: TrendingUp, text: t('premium.ai_programs.bullet_2') },
                  { icon: Target, text: t('premium.ai_programs.bullet_3') },
                ].map((item) => (
                  <li key={item.text} className="flex items-start gap-3">
                    <item.icon className="w-5 h-5 text-accent shrink-0 mt-0.5" aria-hidden="true" />
                    <span className="text-sm text-body">{item.text}</span>
                  </li>
                ))}
              </ul>
            </div>

            <ScreenshotCarousel
              maxHeight="max-h-[420px]"
              fit="contain"
              dotColor="accent"
              images={[
                { src: '/images/screenshot-program-objectif.webp', alt: t('premium.ai_programs.carousel_goal_alt') },
                { src: '/images/screenshot-program-profil.webp', alt: t('premium.ai_programs.carousel_profile_alt') },
                { src: '/images/screenshot-program-config.webp', alt: t('premium.ai_programs.carousel_config_alt') },
              ]}
            />
          </div>
        </div>
      </section>

      {/* ── 4. Tarifs ── */}
      <section id="tarifs" className="px-6 md:px-10 lg:px-14 py-14 md:py-20 bg-surface-2/50 scroll-mt-20">
        <div className="max-w-5xl mx-auto">
          <h2 className="font-display text-2xl md:text-3xl font-black text-heading text-center mb-10">
            {t('premium.pricing_heading')}
          </h2>
          <PricingCards />
        </div>
      </section>
    </div>
  );
}
